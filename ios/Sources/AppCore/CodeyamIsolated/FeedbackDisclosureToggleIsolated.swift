...
//
// The toggle is the shared feedback disclosure header, titled by its caller; its state
// is a constant binding here (isolated rendering is about appearance, not the tap). The
...
// over its disclosed section. The Counter Settings title is used here since that panel
// discloses the fuller set of rows.
struct FeedbackDisclosureToggleIsolated: View {
    ...
    private var revealedRows: some View {
        VStack(alignment: .leading, spacing: 18) {
            ForEach(["HANDEDNESS", "SOUND", "INCREMENT HAPTIC", "DECREMENT HAPTIC"], id: \.self) { label in
